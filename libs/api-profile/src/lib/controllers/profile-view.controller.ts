...
export class ProfileViewController {
  ...
  @Get('/:viewId')
  async getView(@Param('viewId') viewId: number): Promise<ViewEntity> {
    return await this.viewSerivce.getView(viewId)
  }

  @Post('/:resourceId')
  async createView(@Param() resourceId: number): Promise<ViewEntity> {
  ...
  @Patch()
  async editView(@Body() view: any): Promise<ViewEntity> {
