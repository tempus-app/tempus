...
export class CertificationEntity implements Certification {
	constructor(id?: number, title?: string, institution?: string, summary?: string, resource?: ResourceEntity) {
		this.id = id;
		this.title = title;
	...
	public static fromDto(dto: CreateCertificationDto): CertificationEntity {
		...
		return new CertificationEntity(undefined, dto.title, dto.institution, dto.summary);
	}
}
