...
export class EditTimesheetComponent implements OnInit {
	...
	updateTimesheet() {
		const newGeneratedTimesheet = this.newTimesheetForm.editTimesheet();
	...
	openSubmitConfirmation() {
		this.translateService
			.get([`onboardingResourceEditTimesheet.modal.submitModal`])
			.pipe(take(1))
			.subscribe(data => {
				const dialogText = data[`onboardingResourceEditTimesheet.modal.submitModal`];
