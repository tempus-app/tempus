...
@Component({
	selector: 'tempus-admin-management',
...
export class AdminManagementComponent implements OnInit, OnDestroy {
	...
	prefix = 'onboardingOwnerAdminManagement.';
	...
	adminManagementForm = this.fb.group({
		search: [''],
		invite: this.fb.group({
			...
			position: [
				...
				this.requiredIfValidator(
					() =>
						this.adminManagementForm.get('invite')?.get('inviteType')?.value ===
	...
	get inviteType() {
		return this.adminManagementForm.get('invite')?.get('inviteType')?.value;
	}
	...
	updateProjects = (clientId?: string) => {
		...
		this.adminManagementForm.get('invite')?.get('project')?.reset();
	...
	invite() {
		...
		this.adminManagementForm
			.get('invite')
			?.get('inviteType')
			?.valueChanges.pipe(distinctUntilChanged())
			.subscribe(() => {
				this.adminManagementForm.get('invite')?.get('project')?.reset();
				this.adminManagementForm.get('invite')?.get('position')?.reset();
				this.adminManagementForm.get('invite')?.get('client')?.reset();
			});
		...
		this.adminManagementForm
			...
			?.valueChanges.pipe(
				...
				finalize(() => {
					const createLinkDto = {
						firstName: this.adminManagementForm.get('invite')?.get('firstName')?.value,
						lastName: this.adminManagementForm.get('invite')?.get('lastName')?.value,
						email: this.adminManagementForm.get('invite')?.get('emailAddress')?.value,
					} as ICreateLinkDto;
					...
					if (inviteType === RoleType.AVAILABLE_RESOURCE) {
						...
						(createLinkDto.projectId = this.adminManagementForm.get('invite')?.get('project')?.value),
			...
			.subscribe(() => {
				if (this.adminManagementForm.get('invite')?.valid) {
	...
	resetModalData = () => {
		...
		this.adminManagementForm.get('invite')?.reset();
	};
