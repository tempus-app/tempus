...
export class ResumeUploadComponent implements OnInit {
	...
	async parseFile(file: File) {
		...
		return this.http.post<ParsedResume>('python-api/parse', formData);
	}

	async onUpload(file: File, makeRequest: boolean) {
		if (makeRequest) {
			(await this.parseFile(file)).subscribe((data: ParsedResume) => {
				this.createUserDetails(data);
				this.createWorkDetails(data);
				this.createTrainingandSkilsDetails(data);
